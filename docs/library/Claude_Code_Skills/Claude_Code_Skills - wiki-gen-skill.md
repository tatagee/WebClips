...
title: "wiki-gen-skill.md"
source: "unknown"
clipped_at: 2026-04-06
category: Claude_Code_Skills
tags: ['claude-code-skill', 'wiki-compiler']
status: distilled
compiled: true
---
...
Each note becomes a file. Preserve frontmatter. Extract: title, date (from frontmatter or filename), tags, body. Convert `[wikilinks](/wiki/concepts/wikilinks)` to plain text for raw entries.
...
2. Rebuild `_backlinks.json` (Python script scanning `[wikilinks](/wiki/concepts/wikilinks)`)
...
3. **Read 3-8 relevant articles.** Follow `[wikilinks](/wiki/concepts/wikilinks)` and `related:` entries 2-3 links deep when relevant.
...
related: ["[Other Article](/wiki/concepts/Other%20Article)", "[Another](/wiki/concepts/Another)"]
...
Use `[wikilinks](/wiki/concepts/wikilinks)` between articles. Cite sources in frontmatter using entry IDs.
