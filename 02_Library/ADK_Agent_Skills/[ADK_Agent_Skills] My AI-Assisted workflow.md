...
source: "https://www.maiobarbero.dev/articles/ai-assisted-workflow/#step-1-free-form-plan"
clipped_at: "2026-04-18"
category: "ADK_Agent_Skills"
tags: ["workflow", "prd", "ai-assisted"]
status: "distilled"
compiled: true
---
...
This workflow has been adapted to work with me from Mark Pocock’s [skills](https://github.com/mattpocock/skills/tree/main).
...
The quality of everything downstream depends entirely on the quality of this step. A vague plan produces a vague **[PRD](https://www.maiobarbero.dev/glossary/#prd)**, which produces vague issues, which produces code that technically runs but doesn’t do what you meant.
...
The **PRD** becomes a set of issues using **[vertical slices](https://www.maiobarbero.dev/glossary/#vertical-slice)**, tracer bullets that cut through every integration layer end-to-end rather than horizontal slices of a single layer. A slice that only touches the database, or only touches the UI, is not a valid slice. Each issue should deliver a narrow but complete path that is demoable or verifiable on its own. Each issue is classified as either **[AFK](https://www.maiobarbero.dev/glossary/#afk)** (the AI can implement and the change can be merged without human interaction) or **[HITL](https://www.maiobarbero.dev/glossary#hitl)** (a human decision is required at some point during implementation). Preferring AFK over HITL wherever possible keeps the work moving without becoming a bottleneck on my attention.
...
Now if you read this article to the end you at least deserve a link with my skills. Check my [GitHub repo](https://github.com/maiobarbero/my-ai-workflow).